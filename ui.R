...
shinyUI(pageWithSidebar( 
  ...
  sidebarPanel(
    ...
    conditionalPanel(
      ...
      selectInput("linearModel", "Linear or smoothed", #選了smoother會顯示的
                  list("lm", "loess"))
    ),
    ...
    sliderInput("animation", "Trend over time",
                ...
                animate=animationOptions(interval=1000, loop=FALSE)), #毫秒
    
   uiOutput("reacDomains")
    
  ),
  mainPanel(
    tabsetPanel(id ="theTabs", #上面那一排的字
                tabPanel("Summary", textOutput("textDisplay"),
                         textOutput("queryText"), value = "summary"), #value是輸入server
