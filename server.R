...
shinyServer(function(input, output, session){ # pass in a session argument
  ...
  # keep control of the checkbox control 當都沒選的時候選NHS
  ...
  output$animateGraph <- renderPlot({
    ...
    smoothData = graphData[graphData$Date %in% #是否包含
                             quantile(graphData$Date, input$animation/100, type=1): #取正整數
                             quantile(graphData$Date, (input$animation+20)/100, type=1),]
  ...
  output$downloadData.trend <- downloadHandler(
    ...
    content <- function(file) {
      png(file, width = 980, height = 400,
          ...
          bg = "white", res = NA) #ppi 未指定，則設置為72 ppi以設置文本和線寬的大小
